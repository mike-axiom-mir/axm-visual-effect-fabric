...
test('volumetric-light mode emits haze, beam, selective core and tip-fade structure instead of equal bright lines', () => {
  const result = run({
    presentationMode: 'volumetric-light',
    hazeWidthMultiplier: 7.5,
    beamWidthMultiplier: 2.8,
  ...
  assert.ok(coreCount < rayCount, 'not every ray should receive a bright core');
  assert.equal((svg.match(/data-layer="ray-haze"/g) ?? []).length, 1);
...
test('volumetric renderer is caller-neutral and its shaping controls are disposable', () => {
  const human = run({ presentationMode: 'volumetric-light', coreThreshold: 0.5, hazeBlur: 2.5 }, 'human');
  const machine = run({ presentationMode: 'volumetric-light', coreThreshold: 0.5, hazeBlur: 2.5 }, 'machine');
